...
local function local_config_test()
	--local f = assert(io.open("Config/Role.json"), "Can't open file")
	local f = io.open("/mnt/k/IMDT/Skynet-Develop/lesson6/excel_tool/server/Config/Npc.json")

	local str = f:read "a"
	f:close()
	local roleConfig = json.decode(str)
	print(roleConfig[1].id)
	print(roleConfig[1].BaseAttack)
	print(roleConfig[1].BaseDefense)
	send_request(proto_pack("new_npc",{ id = roleConfig[1].id, BaseAttack = roleConfig[1].BaseAttack, BaseDefense = roleConfig[1].BaseDefense}), client_fd)
